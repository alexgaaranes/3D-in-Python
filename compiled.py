...
while running:
    ...
    z_points = []
    for i in range(8):
        rotatedPoint = matMult(identity,cube[i])
        
        screen.fill(WHITE)
        if x_rot:
            rotatedPoint = matMult(rotateX,rotatedPoint)
        if nx_rot:
            rotatedPoint = matMult(rotateNX,rotatedPoint)
        if y_rot:
            rotatedPoint = matMult(rotateY,rotatedPoint)
        if ny_rot:
            rotatedPoint = matMult(rotateNY,rotatedPoint)
        if z_rot:
            rotatedPoint = matMult(rotateZ,rotatedPoint)
        if nz_rot:
            rotatedPoint = matMult(rotateNZ,rotatedPoint)

        cube[i] = rotatedPoint
        z = rotatedPoint[2][0]
        #print(z)
        z_points.append(z)
        projectedPoint = matMult(projectionMat,rotatedPoint)
